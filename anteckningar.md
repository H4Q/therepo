
## Idéer

* oNLine System for Sweden
* Aggressive git
* RSS to git data source
* Principal component decomposition och andra statistiska metoder
* Azure tutorials
* Reddit voting

## Exempel

http://resource.sgu.se/oppnadata/dokument/html/
	stoppa in http://resource.sgu.se/oppnadata/geokemi/api/atlas/element/pb?format=json i http://resource.sgu.se/oppnadata/dokument/html/geojson-viewer.html
	http://resource.sgu.se/oppnadata/dokument/html/osm-wms.html
	
https://sv.wikipedia.org/wiki/G%C3%B6singen

Johan Frisks kodexempel

https://github.com/Hypergene/kolada

https://www.havochvatten.se/hav/uppdrag--kontakt/vart-uppdrag/evenemang-och-seminarier/datavardstraff-2014.html
	https://www.havochvatten.se/download/18.549ab516149e19df88f7f970/1418197003474/14+Wikipedia+Naturvardsverket14.pdf
	
	http://position2015.se/

## Strategi

"Donera din kod!

Allmänheten behöver ... 

http://h4q.github.io/"

